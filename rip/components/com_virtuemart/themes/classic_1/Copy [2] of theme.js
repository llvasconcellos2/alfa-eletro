	...
	var req = new Request({
		...
		onComplete: function(response) { 
			...
			if(BrowserDetect.browser == "Explorer"){
				titulo += '<div style="position: absolute; z-index:9999; width:360px;height:130px;margin-top:26px;margin-left:-55px;">'+texto+'<p style="text-align:center"><input type="button" value="'+ok_lbl+'" onclick="$(\'modalWindowContainer\').destroy(); $(\'modalWindowWrapper\').destroy();clearTimeout(t);" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value="'+cart_title+'" onclick="handleGoToCart()" /></p>'+'</div>';
				texto = "";
			}
			else texto += '<p style="text-align:center"><input type="button" value="'+ok_lbl+'" onclick="document.boxB.close();clearTimeout(t);" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" value="'+cart_title+'" onclick="handleGoToCart()" /></p>';
			...
			t = setTimeout( 'document.boxB.close()', 5000 );
...
function abreFormEnvio(validate) {
	...
	var texto = '<div style="background-color:#ffffff;padding:4px;">Para finalizar o seu pedido de or&ccedil;amento preencha o formul&aacute;rio abaixo:<form action="http://www.alfaeletro.com.br/index.php" method="post" name="adminForm" id="adminForm">';
	...
	texto += '<input type="hidden" name="user_id" value="59" />';
	...
	texto += '<input type="hidden" name="func" value="checkoutProcess" />';
	texto += '<input type="hidden" name="page" value="checkout.thankyou" />';
	texto += '<input type="hidden" name="zone_qty" value="2" />';
	texto += '<input type="hidden" name="ship_to_info_id" value="6018feda7c66aa46b183e94094e1dba1" />';
	texto += '<input type="hidden" name="shipping_rate_id" value="flex%7CSTD%7CStandard+Shipping+under+25.00%7C12" />';
	texto += '<input type="hidden" name="payment_method_id" value="2" />';
	texto += '<input type="hidden" name="checkout_last_step" value="4" />';
	texto += '<input type="hidden" name="checkout_this_step[]" value="CHECK_OUT_GET_FINAL_CONFIRMATION" />';
	texto += '</form></div>';
...
function handleAddUserAjax(formId) {
	...
	var req = new Request({
		...
		onComplete: function(response) {
			...
			temp = new String(response);
			temp = temp.split('ship_to_info_id" value="');
			alert(temp[1]);
