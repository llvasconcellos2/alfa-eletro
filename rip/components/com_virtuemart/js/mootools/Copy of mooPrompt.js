var MooPrompt = box = new Class({
	...
	initialize: function(title, content, options){
		...
		if (this.options.overlay) {
			this.fx1 = new Fx.Tween(this.overlay, 'opacity', {duration:500}).start(0, .8);
		}
		if (this.options.effect == 'grow') {
			...
			this.fx2.custom({
				...
				new Fx.Tween(this.box, 'opacity', {duration: 500}).custom(0, 1).chain(function() {
	...
	close: function(fn) {
		...
		if (this.options.overlay) {new Fx.Tween(this.overlay, 'opacity', {duration:250}).custom(.8, 0);}
		new Fx.Tween(this.container, 'opacity', {
			duration:250,
			onComplete: function() {
